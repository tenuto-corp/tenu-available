...
export default class Day extends PureComponent {
  constructor({ initialSelections,props}) {
    super(props);
    this.state = {
      index: undefined,
   ...
   // const ROUND_TO_NEAREST_MINS = this.props.timeUnit;
  // const MINILENGTH_MINUTE = this.props.timeUnit;
  }
  ...
  relativeY(pageY, rounding = this.props.timeUnit) {
      ...
      for (let i = 0; i < selections.length; i++) {
        if (selections[i].start === start && selections[i].end === end) {
          return {
            ...
            minLengthInMinutes: this.props.timeUnit,
  ...
  handleMouseDown(e) {
    ...
    const position = this.relativeY(e.pageY,this.props.timeUnit);
    ...
    let end = toDate(this.props.date, position + (this.props.timeUnit*MINUTE_IN_PIXELS), timeZone);
    ...
    this.setState(({ minLengthInMinutes, selections, edge, index, lastKnownPosition, target }) => {
      ...
      if (edge === 'both') {
        ...
        if (minPos < position) {
          ...
          newMinLength = this.props.timeUnit;
        }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        {selections.map(({ start, end, status}, i) => 
          <TimeSlot
            // eslint-disable-next-line react/no-array-index-key
            key={i}
            timeConvention={timeConvention}
